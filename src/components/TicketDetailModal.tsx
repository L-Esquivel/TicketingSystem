...
export function TicketDetailModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(`Incidencia ${ticket.ticketNumber} actualizada correctamente`);
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    if (!ticket) return;
    if (!confirm(`¿Estás seguro de eliminar el ticket ${ticket.ticketNumber}?`)) return;

    try {
      const res = await fetch(`/api/tickets/${ticket.id}`, { method: 'DELETE' });
      if (res.ok) {
        toast.success(`Ticket ${ticket.ticketNumber} eliminado`);
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">
                  {ticket?.company?.name || 'Cargando...'}
                </h2>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Registrado {formatDate(ticket?.createdAt)} ({formatTimeAgo(ticket?.createdAt)})
              </p>
            </div>
          </div>
            ...
            <div className="lg:col-span-2 p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <span className="text-[11px] text-slate-400 block font-medium">Solicitante</span>
              ...
              <div className="space-y-4 pt-2">
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-400 mb-1 flex items-center gap-1.5">
                    ...
                    Notas de Solución / Resolución de IT (Públicas/Cliente)
                  </label>
                  <textarea
                    rows={3}
                    value={resolutionNotes}
                    onChange={(e) => setResolutionNotes(e.target.value)}
                    placeholder="Describe los pasos realizados para solucionar la falla, parches aplicados o instrucciones para el cliente..."
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-1 flex items-center gap-1.5">
                    ...
                    Notas Internas de Soporte (Solo para tu registro privado)
                  </label>
                  <textarea
                    rows={2}
                    value={internalNotes}
                    onChange={(e) => setInternalNotes(e.target.value)}
                    placeholder="Notas técnicas confidenciales, credenciales temporales, números de ticket de proveedores..."
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="WAITING">⏸️ En Espera de Cliente/Proveedor</option>
                ...
                <div className="pt-4 border-t border-slate-200 dark:border-slate-800">
                  ...
                  <div className="space-y-3 max-h-56 overflow-y-auto pr-1">
                    {ticket.history && ticket.history.length > 0 ? (
                      ...
                      <p className="text-xs text-slate-400">Sin historial adicional.</p>
                    )}
                  </div>
                </div>
              </div>
